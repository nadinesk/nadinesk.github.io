...
t <- merge(trend_words_sentiment, bh_trend_words_sentiment, by="sentiment")

str(mt)

mt_melt <- melt(mt[,c('sentiment', 'perc.x', 'perc.y')])
...
mt$rhoa_perc <- as.numeric(paste(as.numeric(format(round(mt$rhoa_perc,2), nsmall = 2)) * 100, "%", sep=""))
mt$rhobh_perc <- as.numeric(paste(as.numeric(format(round(mt$rhobh_perc,2), nsmall = 2)) * 100, "%", sep=""))
